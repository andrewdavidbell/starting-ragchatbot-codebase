...
class TestAPIIntegration:
    ...
    def test_query_endpoint_rag_error(self, client, mock_rag_system):
        ...
        response = client.post("/api/query", json={"query": "What causes error?"})
    ...
    def test_request_response_models(self, client, mock_rag_system):
        ...
        response = client.post(
            "/api/query", json={"query": "Test query", "session_id": "optional_session"}
        )
    ...
    def test_concurrent_requests(self, client, mock_rag_system):
        ...
        for i in range(5):
            response = client.post(
                "/api/query", json={"query": f"Concurrent query {i}"}
            )
    ...
    def test_session_persistence(self, client, mock_rag_system):
        ...
        response2 = client.post(
            "/api/query", json={"query": "Second query", "session_id": session_id}
        )
        ...
        assert (
            calls[0][0][1] == session_id
        )  # Second argument (session_id) of first call
        assert (
            calls[1][0][1] == session_id
        )  # Second argument (session_id) of second call
...
class TestAPIRealIntegration:
    ...
    def test_real_query_endpoint(self, real_client):
        ...
        response = real_client.post("/api/query", json={"query": "What is 2+2?"})
