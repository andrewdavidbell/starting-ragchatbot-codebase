...
class TestAIGenerator:
    ...
    def test_generate_response_with_tools_no_tool_use(
        self, mock_anthropic_client, test_config, tool_manager
    ):
    ...
    def test_generate_response_with_tool_use(
        self, mock_anthropic_with_tools, test_config, tool_manager
    ):
        ...
        tools = tool_manager.get_tool_definitions()
        response, sources = ai_gen.generate_response(
            "Search for Python content", tools=tools, tool_manager=tool_manager
        )
    ...
    def test_tool_execution_multiple_tools(self, test_config):
        ...
        with patch("anthropic.Anthropic") as mock_client_class:
            ...
            mock_tool_manager.execute_tool.assert_any_call(
                "search_course_content", query="JavaScript"
            )
    ...
    @pytest.mark.unit
    def test_no_tool_manager_with_tools(
        self, mock_anthropic_with_tools, test_config, tool_manager
    ):
        ...
        response, sources = ai_gen.generate_response(
            "Search query", tools=tools, tool_manager=None
        )
    ...
    def test_sequential_two_round_tool_execution(self, test_config, tool_manager):
        ...
        with patch("anthropic.Anthropic") as mock_client_class:
            ...
            response, sources = ai_gen.generate_response(
                "What course covers variables?", tools=tools, tool_manager=tool_manager
            )
    ...
    def test_early_termination_single_round(self, test_config, tool_manager):
        ...
        with patch("anthropic.Anthropic") as mock_client_class:
            ...
            mock_text_block.text = (
                "Python is a programming language covered in course X."
            )
    ...
    def test_max_rounds_final_synthesis(self, test_config, tool_manager):
        ...
        with patch("anthropic.Anthropic") as mock_client_class:
            ...
            response, sources = ai_gen.generate_response(
                "Complex multi-part query", tools=tools, tool_manager=tool_manager
            )
    ...
    def test_message_array_structure_multi_round(self, test_config, tool_manager):
        ...
        with patch("anthropic.Anthropic") as mock_client_class:
            ...
            final_call_messages = mock_client.messages.create.call_args_list[2][1][
                "messages"
            ]
    ...
    def test_system_prompt_multi_round_instructions(self, test_config):
        ...
        assert (
            "Synthesize ALL search results" in prompt or "synthesize" in prompt.lower()
        )
    ...
    def test_real_api_integration(self, test_config):
        ...
        if (
            not test_config.ANTHROPIC_API_KEY
            or test_config.ANTHROPIC_API_KEY == "test_api_key"
        ):
