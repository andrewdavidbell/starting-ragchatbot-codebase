...
class TestSystemDiagnostics:
        ...
        for query in test_queries:
            try:
                ...
                if not results.is_empty():
                    print(f"✓ Search for '{query}' returned {len(results.documents)} results")
                else:
                    print(f"✗ Search for '{query}' returned no results. Error: {results.error}")
            except Exception as e:
    ...
    def test_course_name_resolution(self):
        ...
        if resolved != first_course:
            pytest.fail(f"✗ Course name resolution failed. Expected: {first_course}, Got: {resolved}")
        else:
        ...
        if len(first_course.split()) > 1:
            ...
            if resolved_partial:
                print(f"✓ Course name resolution works for partial match: '{partial_name}' -> '{resolved_partial}'")
            else:
                print(f"✗ Course name resolution failed for partial match: '{partial_name}'")
    ...
    def test_search_tool_functionality(self):
        ...
        try:
            ...
            if "No relevant content found" in result:
                ...
                print(f"✓ CourseSearchTool search successful: {len(result)} characters returned")
    ...
    def test_anthropic_api_connectivity(self):
        ...
        try:
            ...
            if not response or len(response.strip()) == 0:
                ...
                print(f"✓ Anthropic API connection successful. Response: {response[:100]}...")
    ...
    def test_anthropic_tool_calling(self):
        ...
        try:
            ...
            if not response:
                ...
                print(f"✓ Anthropic API tool calling works. Response: {response[:150]}...")
        ...
        for test_case in test_cases:
            ...
            try:
                ...
                if "failed query" in response.lower():
                    pytest.fail(f"✗ Got 'failed query' response for: {test_case['query']}")
                
            except Exception as e:
                pytest.fail(f"✗ End-to-end query failed for '{test_case['query']}': {e}")
    ...
    def test_diagnose_failed_query_issue(self):
        ...
        tool_manager = ToolManager()
        tool_manager.register_tool(search_tool)
        manager_result = tool_manager.execute_tool("search_course_content", query="introduction")
    ...
    def test_document_integrity(self):
        ...
        if course_count == 0:
            pytest.fail("No courses loaded in VectorStore despite having document files")
        else:
            print(f"✓ {course_count} courses loaded from {len(doc_files)} document files")
    ...
    def test_environment_configuration(self):
        ...
        print(f"ANTHROPIC_API_KEY configured: {'Yes' if config.ANTHROPIC_API_KEY and config.ANTHROPIC_API_KEY != 'test_api_key' else 'No'}")
