...
class TestRAGSystem:
    ...
    def test_add_course_document_success(self, mock_search_tool, mock_session, mock_ai, mock_vector, mock_doc_processor, test_config):
        ...
        mock_doc_processor.return_value.process_course_document.return_value = (mock_course, mock_chunks)
        ...
        mock_doc_processor.return_value.process_course_document.assert_called_once_with("/path/to/test.txt")
    ...
    @patch('rag_system.SessionManager')
    @patch('rag_system.CourseSearchTool')
    def test_add_course_document_error(self, mock_search_tool, mock_session, mock_ai, mock_vector, mock_doc_processor, test_config):
        """Test error handling when adding course document fails"""
        # Setup mock to raise exception
        mock_doc_processor.return_value.process_course_document.side_effect = Exception("Processing failed")
    ...
    @patch('os.listdir')
    def test_add_course_folder_success(self, mock_listdir, mock_exists, mock_search_tool, mock_session, mock_ai, mock_vector, mock_doc_processor, test_config):
        ...
        mock_chunks1 = [CourseChunk(content="Content 1", course_title="Course 1", lesson_number=1, chunk_index=0)]
        mock_chunks2 = [CourseChunk(content="Content 2", course_title="Course 2", lesson_number=1, chunk_index=0)]
    ...
    @patch('rag_system.CourseSearchTool')
    @patch('os.path.exists')
    def test_add_course_folder_nonexistent(self, mock_exists, mock_search_tool, mock_session, mock_ai, mock_vector, mock_doc_processor, test_config):
    ...
    @patch('rag_system.SessionManager')
    @patch('rag_system.CourseSearchTool')
    def test_query_without_session(self, mock_search_tool, mock_session, mock_ai, mock_vector, mock_doc_processor, test_config):
    ...
    @patch('rag_system.SessionManager')
    @patch('rag_system.CourseSearchTool')
    def test_query_with_session(self, mock_search_tool, mock_session, mock_ai, mock_vector, mock_doc_processor, test_config):
        ...
        mock_tool_manager.get_last_sources.return_value = [{"text": "Source 1", "link": None}]
        ...
        mock_session_instance.get_conversation_history.assert_called_once_with("session_1")
    ...
    @patch('rag_system.SessionManager')
    @patch('rag_system.CourseSearchTool')
    def test_query_with_tool_usage(self, mock_search_tool, mock_session, mock_ai, mock_vector, mock_doc_processor, test_config):
        """Test query that triggers tool usage"""
        # Setup mocks for tool usage scenario
        mock_ai_instance = Mock()
        mock_ai.return_value = mock_ai_instance
        mock_ai_instance.generate_response.return_value = "Based on search results: Python is a programming language"
    ...
    @patch('rag_system.SessionManager')
    @patch('rag_system.CourseSearchTool')
    def test_get_course_analytics(self, mock_search_tool, mock_session, mock_ai, mock_vector, mock_doc_processor, test_config):
    ...
    @patch('os.path.exists')
    @patch('os.listdir')
    def test_skip_existing_courses(self, mock_listdir, mock_exists, mock_search_tool, mock_session, mock_ai, mock_vector, mock_doc_processor, test_config):
        ...
        mock_doc_processor.return_value.process_course_document.side_effect = [
            (mock_course1, []),
            (mock_course2, [CourseChunk(content="New content", course_title="New Course", lesson_number=1, chunk_index=0)])
        ]
        ...
        total_courses, total_chunks = rag.add_course_folder("/test/folder", clear_existing=False)
    ...
    @patch('rag_system.CourseSearchTool')
    @patch('os.path.exists')
    def test_clear_existing_data(self, mock_exists, mock_search_tool, mock_session, mock_ai, mock_vector, mock_doc_processor, test_config):
    ...
    @pytest.mark.slow
    @pytest.mark.integration
    def test_full_workflow_with_test_data(self, test_config, sample_course, sample_course_chunks):
        ...
        tool_result = rag.tool_manager.execute_tool("search_course_content", query="introduction")
